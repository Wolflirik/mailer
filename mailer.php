				...
				if($query->row['count'] == 0 && isset($results->orders[$i]->order_id)){
					...
					if(!empty($order->delivery_num)){
						$this->db->query("INSERT INTO `orders` (`order_id`, `email`, `status`, `delivery_id`, `full_name`, `track`, `date_created`, `user_phone`, `address_string`, `delivery_method`, `first_name`, `text_map`) VALUES (
							...
							'".$this->db->esc(trim($order->recipient->email))."', 
	...
	public function mail_send(){
		...
		foreach($results->rows as $row){
			...
			$replacements = array(
				...
				$row['first_name'],
				$row['text_map'],
				'<div style="color:#000;font-size:15px;margin:0;"',
				'<p style="color:#000;font-size:15px;margin:0;"',
				'<p style="color:#000;font-size:15px;margin:0;"'
			);
			$search = array(
			    '%delivery_name%',
			...
			if($row['email'] != ''){
				...
				$this->db->query("UPDATE `orders` SET `email_status` = '2' WHERE `orders`.`id` = '".$row['id']."'");
			}
		}
	}
}
